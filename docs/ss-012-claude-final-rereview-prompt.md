# SS-012 Claude Final Re-Review Prompt

Role: You are Claude performing a focused final implementation re-review for Swing Sync SS-012 after your prior final audit returned FAIL.

Stage: focused implementation re-review after fixes.

Scope: Review only the B9/B10 fixes and any directly related regression risk in the coaching response contract, validation context derivation, and unit tests. Do not re-open unrelated previously passed planning issues unless the B9/B10 fix creates a new blocker.

Claude Chat does not have filesystem or GitHub access, so this prompt includes the complete current contents of the implementation and test files relevant to this re-review.

## Project Boundaries

Swing Sync is local-first. Raw swing video is not uploaded by default. This story must not add provider SDKs, remote model/API calls, API keys, telemetry, remote logging, cloud storage, public serving, model assets, new workers, or new dependencies.

The coaching prompt and response contract are educational golf movement support only. They must not make medical, injury, rehabilitation, professional coaching, guaranteed correctness, guaranteed privacy/deletion/anonymity, legal, or compliance claims.

## Prior Final Audit Findings

Your prior SS-012 implementation audit returned FAIL:

- B9: `supported`/`limited` distinction for partial-evidence phases was unenforced. A phase with partial overlay evidence and no measured metric could pass validation with `evidenceStatus: "supported"`.
- B10: The partial-overlay test only proved `limited` was accepted; it did not assert that `supported` was rejected in that same limited-evidence scenario.

You also noted two watch items:

- Confirm `limited` is not a third silently-maintained mutable list that can drift from evidence classification.
- Confirm `coachingProhibitedTextPatterns.description` does not leak through validation or UI paths.

## Applied Fix

Codex addressed B9/B10 by revising `src/coaching-contract.ts`:

- Added `LIMITED_EVIDENCE_REQUIRES_LIMITED_STATUS` to the exhaustive `CoachingValidationErrorCode` union and deterministic `errorOrder`.
- Added `limitedPhaseIds` to `CoachingValidationContext`.
- Updated `buildCoachingValidationContext(content: SwingCardContent)` to derive `limitedPhaseIds` from actual Swing Card content:
  - no keyframe evidence and no measured metric => unavailable;
  - some evidence but incomplete evidence => limited;
  - rendered keyframe evidence and measured metric => neither unavailable nor limited.
- Updated `validateEvidenceText` to reject `supported` items for phases in `context.limitedPhaseIds` with `LIMITED_EVIDENCE_REQUIRES_LIMITED_STATUS`.
- Preserved review-required priority over unavailable/limited derivation.

Codex addressed B10 in `test/unit/coaching-prompt.test.ts`:

- The partial-overlay test now asserts `top` is not unavailable, `top` is in `limitedPhaseIds`, a `limited` response for `top` passes, and a `supported` response for `top` fails with `LIMITED_EVIDENCE_REQUIRES_LIMITED_STATUS`.
- Added metric-only evidence coverage: `impact` with measured metric and no keyframe is limited, `top` with neither metric nor keyframe is unavailable, and `supported` for metric-only `impact` fails with `LIMITED_EVIDENCE_REQUIRES_LIMITED_STATUS`.

## Verification After Fix

- `npm run test:unit -- coaching-prompt` passed (13 tests).
...
Please answer with PASS or FAIL.

Focus on:

- Does the revised `limitedPhaseIds` derivation close B9 without creating a new second source of truth or drift-prone classification?
- Does the validator now reject `supported` for partial-overlay and metric-only limited-evidence phases?
- Does review-required priority still prevent supported/limited claims when phase review is required?
- Do the new tests close B10 in both the partial-overlay and metric-only directions?
- Did the fix preserve the no-provider, no-remote, no-telemetry, no-new-dependency, and no raw-video-upload boundaries?
- Are there any new blockers introduced by adding `limitedPhaseIds` and `LIMITED_EVIDENCE_REQUIRES_LIMITED_STATUS`?
...
- Explicit sign-off status for whether SS-012 may proceed to PR preparation.
