> **Superseded: do not paste this prompt.** Claude completed this focused
> re-review with a FAIL result after confirming B1-B6 closed and identifying
> new blocker B7 plus minor B8. Use
> `docs/ss-012-claude-qa-rereview-2-prompt.md` for the focused B7/B8
> re-review.

Role: You are the lead adversarial QA planner for Swing Sync.
Stage: Focused pre-implementation QA re-review for sensitive story
...
prior findings, Codex response, and full revised specification text.

## Current State

Repository: `https://github.com/ajason13/swing-sync`
Branch: `ss-012-coach-prompt`
Task status in Notion: `2. QA Planning (Claude)`.

No runtime implementation has started. SS-012 must not move to
`3. In Development (ChatGPT)` until Claude QA planning returns PASS or all
...
Prior QA planning verdict: **FAIL** with six blockers.

- B1: `text` field bound was unspecified.
- B2: section arrays were unbounded.
- B3: `phaseId` was an unconstrained string.
- B4: unavailable/review-required evidence could still carry free-form
  substantive claims.
- B5: stable validation error taxonomy was missing.
- B6: unsafe text detection mechanism was unspecified.

Claude also noted that the deferred SS-006 `observedSeekTimestampMs`
export-exclusion item was correctly not triggered, but should be explicitly
traceable.

## Codex Response

Codex accepts all six blockers as valid and revised
`docs/ss-012-preimplementation-spec.md`.

| ID | Response |
| --- | --- |
| B1 | Added exported `maxCoachingResponseItemTextLength = 280` and `ITEM_TEXT_TOO_LONG`. |
| B2 | Added exported `maxCoachingResponseItemsPerSection = 4` and `ITEM_ARRAY_TOO_LONG`. |
| B3 | Changed `phaseId` to `PhaseId` from `src/phase-review.ts` and enumerated the eight allowed phase IDs. |
| B4 | Added exact exported unavailable/review-required templates plus validation-context checks returning `UNAVAILABLE_TEXT_NOT_TEMPLATE`, `REVIEW_REQUIRED_TEXT_NOT_TEMPLATE`, or `FABRICATED_SUPPORTED_EVIDENCE`. |
| B5 | Added exhaustive exported `CoachingValidationErrorCode` and deterministic `CoachingValidationResult`. |
| B6 | Added exported `coachingProhibitedTextPatterns` data with deterministic pattern checks for medical, rehabilitation, aggressive movement, guarantee, privacy/legal/compliance, hidden payload-key, raw JSON, and coordinate patterns. |

The spec now explicitly states the SS-006 timestamp non-regression: raw
timestamps, including `observedSeekTimestampMs`, are disallowed in prompt
inputs and response text, validation errors, logs, persistence, and network
behavior.
...
Please re-review only the B1-B6 closure and any new cross-cutting risk created
by the revised specification.

Output required:

- PASS or FAIL verdict.
- Whether B1-B6 are closed.
- Any new blockers introduced by the revised contract.
